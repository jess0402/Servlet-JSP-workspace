...
<body>
	...
	<form name="searchFrm">
		...
		<button>검색</button> <!-- button태그는 타입이 지정되지 않으면 기본적으로 submit임. -->
	</form>
	<div id="result"></div>
	
	<script>
	let xhr;  // XMLHttpRequest 객체를 대입할 변수
	document.searchFrm.addEventListener('submit', (e) => {
		e.preventDefault();  // 제출금지. 폼제출은 동기요청이므로 비동기가 아님.
		
		// 1. XMLHttpRequest 객체 생성
		xhr = new XMLHttpRequest();	
		console.log(xhr);
		
		// 2. xhr 객체 연결상태 변화에 따른 핸들러 함수 바인딩
		xhr.onreadystatechange = readStateChangeHandler;
		
		// 3. open 연결
		// open("전송방식", "url?querystring")
		xhr.open("GET", "<%= request.getContextPath() %>/vanilla-js/search?q=" + e.target.q.value);	
		// 사용자 입력값을 url 옆에 붙여서 전송
		
		// 4. send 요청전송
		xhr.send();
	});
	
	const readStateChangeHandler = (e) => {
	...
	document.signupFrm.addEventListener('submit', (e) => {
		...
		// 4. send 요청 - 사용자 입력값을 여기서 전달 (message body에 작성)
		// 데이터 직렬화 해서 보내줘야 함.
		const usernameVal = e.target.username.value; // e.target이 현재 폼
		const emailVal = e.target.email.value;
		const param = `username=\${usernameVal}&email=\${emailVal}`; // 직렬화 처리
		
		xhr.send(param);  // 데이터 직렬화 처리후 전송	

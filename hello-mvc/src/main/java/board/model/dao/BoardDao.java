...
public class BoardDao {
	...
	public int insertBoard(Connection conn, Board board) {
		...
		try {
			...
			throw new BoardException("게시글 삽입 오류!", e);
		} finally {
			close(pstmt);
		}	
		return result;
	...
	public int findCurrentBoardNo(Connection conn) {
		...
		try {
			...
			while(rset.next()) {
				no = rset.getInt(1); // 첫 번째 컬럼 값 주세요
			}
		} catch(SQLException e) {
			throw new BoardException("게시글 번호조회 오류", e);
		} finally {
			close(rset);
			close(pstmt);
		}
		
		System.out.println("BoardDao@no = " + no);
	...
	public Attachment findAttachmentByNo(Connection conn, int no) {
		...
		try {
			...
			throw new BoardException("첨부파일 조회오류", e);
		} finally {
			close(rset);
			close(pstmt);
		}
	...
	public int insertBoardComment(Connection conn, BoardComment bc) {
		...
		try {
			...
			// BoardComment#commentRef 0 ~ n -> 실제 db의 board_comment의 comment_ref는 null ~ n 
			result = pstmt.executeUpdate();
		} catch (SQLException e) {

...
public class BoardService {
	...
	public BoardExt findByNo(int no) {
		...
		BoardExt board = boardDao.findByNo(conn, no);  // board 테이블에서 조회
		List<Attachment> attachments = boardDao.findAttachmentByBoardNo(conn, no); // attachment 테이블에서 조회
	...
	public int updateBoard(BoardExt board) {
		...
		try {
			// 1. board 수정
			result = boardDao.updateBoard(conn, board);  // pk no값 결정 - seq_board_no

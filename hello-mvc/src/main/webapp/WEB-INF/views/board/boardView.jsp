	...
	<div class="comment-container">
		...
		<table id="tbl-comment">
			...
			%>
				<tr class="level1">
					<td>
						<sub class="comment-writer"><%= bc.getMemberId() != null ? bc.getMemberId() : "(탈퇴 회원)"%></sub>
...
<!-- @실습문제 -> .btn-delete 클릭핸들러를 통해 boardCommentDelFrm을 동적으로 전송 -->
...
document.querySelectorAll(".btn-reply").forEach((button) => {
	button.onclick = (e) => {
		...
		frm.method = "POST";
		frm.onsubmit = commentSubmitHandler; // 동적생성한 요소는 핸들러바인도 새로 해야 한다.
		...
		// 생성된 tr 추가
		//parent.insertBefore(tr, ref); // insertBefore이기 때문에 현재 tr이 아니라 다음 tr
		// parent.insertBefore(추가할 요소, 기준 요소);
		
		// 아래 방법도 tr 추가 가능 (begin을 시작태그 end를 종료태그로 보기)

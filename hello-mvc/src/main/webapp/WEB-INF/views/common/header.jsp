...
<%
	...
	if(msg != null)
		session.removeAttribute("msg");  // msg를 1회용으로 만들겠다. - 새로고침할때마다 메세지 나오는 것 고치는 것.
		
	String saveId = null; // 아이디저장을 체크한 경우, memberId 값이 담길 변수
...
<script>
	...
	// 위와 같이 하면 안됨. 스크립트가 로드될때 아래코드 로드가 다 되지 않아서 loginFrm을 모르기 때문.
	// 아래와 같이 해야함.
	window.onload = () => {
<% if(msg != null){ %>
	alert("<%= msg %>");
<% } %>
		...
		document.loginFrm.onsubmit = (e) => {
			...
			if(!/^.{4,}$/.test(memberIdVal)){  
				// 시작하고(^) 아무글자(.)가 4글자 이상{4,}나오고 끝나야한다($)
